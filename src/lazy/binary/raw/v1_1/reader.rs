...
mod tests {
    ...
    fn sexp() -> IonResult<()> {
        ...
        let tests: &[(&[u8], &[IonType])] = &[
            ...
            // ( () () [] )
            (&[0xFB, 0x09, 0xFB, 0x01, 0xC0, 0xB0], &[IonType::SExp, IonType::SExp, IonType::List]),

            // ( $257 )
            (&[0xFB, 0x07, 0xA2, 0x01, 0x00], &[IonType::Symbol]),
    ...
    #[test]
    #[ignore] // TODO: Revisit with struct updates.
    ...
    fn structs() -> IonResult<()> {
        ...
        let tests: &[(&[u8], &[(RawSymbolRef<'_>, IonType)])] = &[
            ...
            (
                // { "foo": 1, $11: 2 }
                &[ 0xFC, 0x15, 0x01, 0xFB, 0x66, 0x6F, 0x6F, 0x61, 0x01, 0x17, 0x61, 0x02],
                &[ ("foo".into(), IonType::Int), (11usize.into(), IonType::Int)],
            ),
            (
                // { "foo": <NOP>, $11: 2 }
                &[ 0xFC, 0x13, 0x01, 0xFB, 0x66, 0x6F, 0x6F, 0xEC, 0x17, 0x61, 0x02],
                &[ (11usize.into(), IonType::Int) ],
            ),
            (
                // { "foo": 2, $11: <NOP> }
                &[ 0xFC, 0x13, 0x01, 0xFB, 0x66, 0x6F, 0x6F, 0x61, 0x02, 0x17, 0xEC],
                &[ ("foo".into(), IonType::Int) ],
            ),
            (
                // { "foo": { $10: 2 }, "bar": 2 }
                &[
                    0xFC, 0x1F, 0x01, 0xFB, 0x66, 0x6F, 0x6F, 0xD3, 0x15, 0x61, 0x02,
