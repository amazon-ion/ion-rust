...
impl<'top, D: Decoder> TdlSExpKind<'top, D> {
    ...
    pub fn of(
        ...
        let operation_name = operation
            .read()?
            .expect_symbol()
            .map_err(|_| IonError::decoding_error("operation name must be a symbol"))?
            .expect_text()
            .map_err(|_| IonError::decoding_error("operation name cannot have unknown text"))?;

        // TDL-only operations that are not in the system macro table.
        static SPECIAL_FORM_NAMES: &[&str] =
            &["literal", "if_none", "if_some", "if_single", "if_multi"];
        ...
        if is_special_form {
            let special_form_macro: &Arc<Macro> = match operation_name {
                ...
                other => unreachable!("unknown name '{}' found in special forms list", other),
            };
...
pub static IF_MULTI_MACRO: LazyLock<Arc<Macro>> =
    LazyLock::new(|| initialize_cardinality_test_macro("if_multi", MacroKind::IfSingle));
