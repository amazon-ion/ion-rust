...
enum IoBufferSource {
    ...
    None,
    // This encoding context is the reader's instance.
    // It holds a valid reference to the input source and thus can create a new IoBuffer.
    Reader(&'static dyn IoBufferHandle),
    // This encoding context was previously cloned from the reader's instance.
    // It holds a previously constructed IoBuffer that can be cheaply cloned.
    IoBuffer(IoBuffer),
}
