...
impl<W: Write> LazyRawBinaryWriter_1_0<W> {
    ...
    fn get_or_allocate_encoding_buffer<'value, 'top>(
        ...
        let pointer = match encoding_buffer_ptr {
            Some(ptr) => *ptr,
            None => {
                *encoding_buffer_ptr = Some(mut_ref_to_ptr(
                    allocator.alloc_with(|| BumpVec::<u8>::new_in(allocator)),
                ));
                encoding_buffer_ptr.unwrap()
            }
        };
        unsafe { ptr_to_mut_ref::<'_, BumpVec<'_, u8>>(pointer) }
    }
